...
ef_image_t *ef_image_new(const int width, const int height){
    ef_image_t *image = (ef_image_t*) malloc(sizeof(ef_image_t));
    if(image == NULL){
        fprintf(stderr, "Error: ef_image_new() - not enough memory !\n");
        exit(1);
    }
    image->width = width;
    image->height = height;  
...
ef_image_t *ef_image_cpy(const ef_image_t *src){
    ef_image_t *dst = ef_image_new(src->width, src->height);
...
ef_color_image_t *ef_color_image_new(const int width, const int height){
    ef_color_image_t *image = (ef_color_image_t*) malloc(sizeof(ef_color_image_t));
    if(image == NULL){
        fprintf(stderr, "Error: ef_color_image_new() - not enough memory !\n");
        exit(1);
    }
    image->width = width;
    image->height = height;  
...
ef_color_image_t *ef_color_image_cpy(const ef_color_image_t *src){
    ef_color_image_t *dst = ef_color_image_new(src->width, src->height);
...
float *ef_gaussian_filter(const float sigma, int *filter_order){
    if(sigma == 0.0f){
        fprintf(stderr, "ef_gaussian_filter() error: sigma is zeros\n");
        exit(1);
    }
    if(!filter_order){
        fprintf(stderr, "ef_gaussian_filter() error: filter_order is null\n");
        exit(1);
    }
    ...
    *filter_order = floor(3*sigma)+1; 
    ...
    if(data == NULL ){
        fprintf(stderr, "ef_gaussian_filter() error: not enough memory\n");
        exit(1);
    }
...
convolution_t *ef_convolution_new(int order, const float *half_coeffs, const int even){
    ...
    if(conv == NULL){
        fprintf(stderr, "Error: ef_convolution_new() - not enough memory !\n");
        exit(1);
    }
    conv->order = order;
...
static void convolve_vert_fast_3(ef_image_t *dst, const ef_image_t *src, const convolution_t *conv){
...
static void convolve_vert_fast_5(ef_image_t *dst, const ef_image_t *src, const convolution_t *conv){
    const int iterline = (src->stride>>2)+1;
    const float *coeff = conv->coeffs;
    //const float *coeff_accu = conv->coeffs_accu;
    v4sf *srcp = (v4sf*) src->data, *dstp = (v4sf*) dst->data;
...
static void convolve_horiz_fast_3(ef_image_t *dst, const ef_image_t *src, const convolution_t *conv){
...
static void convolve_horiz_fast_5(ef_image_t *dst, const ef_image_t *src, const convolution_t *conv){
    const int stride_minus_1 = src->stride-1;
...
void ef_color_image_convolve_hv(ef_color_image_t *dst, const ef_color_image_t *src, const convolution_t *horiz_conv,
    ...
    ef_image_t src_red = {width,height,stride,src->c1}, src_green = {width,height,stride,src->c2}, src_blue = {width,height,stride,src->c3},
    ...
    if(horiz_conv != NULL && vert_conv != NULL){
        ...
        if(tmp_data == NULL){
	        fprintf(stderr,"error ef_color_image_convolve_hv(): not enough memory\n");
	        exit(1);
        }  
        ef_image_t tmp = {width,height,stride,tmp_data};
        // perform convolution for each channel
        ef_convolve_horiz(&tmp, &src_red, horiz_conv);
        ef_convolve_vert(&dst_red, &tmp, vert_conv);
        ef_convolve_horiz(&tmp, &src_green, horiz_conv);
        ef_convolve_vert(&dst_green, &tmp, vert_conv);
        ef_convolve_horiz(&tmp, &src_blue, horiz_conv);
        ef_convolve_vert(&dst_blue, &tmp, vert_conv);
        free(tmp_data);
    }else if(horiz_conv != NULL && vert_conv == NULL){ // only horizontal
        ef_convolve_horiz(&dst_red, &src_red, horiz_conv);
        ef_convolve_horiz(&dst_green, &src_green, horiz_conv);
        ef_convolve_horiz(&dst_blue, &src_blue, horiz_conv);
    }else if(vert_conv != NULL && horiz_conv == NULL){ // only vertical
        ef_convolve_vert(&dst_red, &src_red, vert_conv);
        ef_convolve_vert(&dst_green, &src_green, vert_conv);
        ef_convolve_vert(&dst_blue, &src_blue, vert_conv);
    }
}
...
ef_color_image_t *ef_rgb_to_lab(const ef_color_image_t *im){
    ...
    ef_color_image_t *res = ef_color_image_new(im->width, im->height);
...
/* compute the ef_saliency of a given image */
ef_image_t* ef_saliency(const ef_color_image_t *im, float sigma_image, float sigma_matrix){
    ...
    // smooth image
    ef_color_image_t *sim = ef_color_image_new(width, height);
    float *presmooth_filter = ef_gaussian_filter(sigma_image, &filter_size);
    convolution_t *presmoothing = ef_convolution_new(filter_size, presmooth_filter, 1);
    ef_color_image_convolve_hv(sim, im, presmoothing, presmoothing);
    ef_convolution_delete(presmoothing);
    free(presmooth_filter);
  
    // compute derivatives
    float deriv_filter[2] = {0.0f, -0.5f};
    convolution_t *deriv = ef_convolution_new(1, deriv_filter, 0);
    ef_color_image_t *imx = ef_color_image_new(width, height), *imy = ef_color_image_new(width, height);
    ef_color_image_convolve_hv(imx, sim, deriv, NULL);
    ef_color_image_convolve_hv(imy, sim, NULL, deriv);
    ef_convolution_delete(deriv);
    
    // compute autocorrelation matrix
    ef_image_t *imxx = ef_image_new(width, height), *imxy = ef_image_new(width, height), *imyy = ef_image_new(width, height);
    ...
    float *smooth_filter = ef_gaussian_filter(sigma_matrix, &filter_size);
    convolution_t *smoothing = ef_convolution_new(filter_size, smooth_filter, 1);
    ef_image_t *tmp = ef_image_new(width, height);
    ef_convolve_horiz(tmp, imxx, smoothing);
    ef_convolve_vert(imxx, tmp, smoothing);
    ef_convolve_horiz(tmp, imxy, smoothing);
    ef_convolve_vert(imxy, tmp, smoothing);
    ef_convolve_horiz(tmp, imyy, smoothing);
    ef_convolve_vert(imyy, tmp, smoothing);
    ef_convolution_delete(smoothing);
    free(smooth_filter);
    ...
    ef_image_delete(imxx);
    ef_image_delete(imxy);
    ef_image_delete(imyy);
    ef_color_image_delete(imx);
    ef_color_image_delete(imy);
    ef_color_image_delete(sim);
