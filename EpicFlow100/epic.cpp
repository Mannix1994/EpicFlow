...
static float_image
rectify_corres(const float_image *matches, const int w1, const int h1, const int w2, const int h2, const int n_thread) {
...
/* remove matches coming from a pixel with a low ef_saliency */
static void apply_saliency_threshold(float_image *matches, const ef_color_image_t *im, const float saliency_threshold) {
    ef_image_t *s = ef_saliency(im, 0.8f, 1.0f);
...
static void prefiltering(float_image *matches, const float_image *edges, const int nn, const float threshold,
                         const float coef_kernel, const int n_thread) {
...
#pragma omp parallel for num_threads(n_thread)
    ...
    for (int i = 0; i < matches->ty; i++) {
        if (pow2(seedsvects.pixels[2 * i] - vects.pixels[2 * i]) +
            pow2(seedsvects.pixels[2 * i + 1] - vects.pixels[2 * i + 1]) < th2) {
            memcpy(&tmp.pixels[ii * 4], &matches->pixels[i * 4], sizeof(float) * 4);
            ii += 1;
        }
    }
...
*/
void epic(ef_image_t *flowx, ef_image_t *flowy, const ef_color_image_t *im, const float_image *input_matches,
