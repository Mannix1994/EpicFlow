...
void ef_get_derivatives(const ef_color_image_t *im1, const ef_color_image_t *im2, const convolution_t *deriv,
                        ef_color_image_t *dx, ef_color_image_t *dy, ef_color_image_t *dt,
                        ef_color_image_t *dxx, ef_color_image_t *dxy, ef_color_image_t *dyy, ef_color_image_t *dxt,
                        ef_color_image_t *dyt) {
    // derivatives are computed on the mean of the first image and the warped second image
    ef_color_image_t *tmp_im2 = ef_color_image_new(im2->width, im2->height);
    ...
    ef_color_image_convolve_hv(dx, tmp_im2, deriv, NULL);
    ef_color_image_convolve_hv(dy, tmp_im2, NULL, deriv);
    ef_color_image_convolve_hv(dxx, dx, deriv, NULL);
    ef_color_image_convolve_hv(dxy, dx, NULL, deriv);
    ef_color_image_convolve_hv(dyy, dy, NULL, deriv);
    ef_color_image_convolve_hv(dxt, dt, deriv, NULL);
    ef_color_image_convolve_hv(dyt, dt, NULL, deriv);
    // free memory
    ef_color_image_delete(tmp_im2);
}
...
void ef_compute_smoothness(ef_image_t *dst_horiz, ef_image_t *dst_vert, const ef_image_t *uu, const ef_image_t *vv,
  ...
  ef_image_t *ux1 = ef_image_new(w, h), *uy1 = ef_image_new(w, h), *vx1 = ef_image_new(w, h), *vy1 = ef_image_new(w, h),
    *ux2 = ef_image_new(w, h), *uy2 = ef_image_new(w, h), *vx2 = ef_image_new(w, h), *vy2 = ef_image_new(w, h);
  ...
  // compute ux2, uy2, vx2, vy2, filter [-0.5 0 0.5]
    ef_convolve_horiz(ux2, uu, deriv_flow);
    ef_convolve_horiz(vx2, vv, deriv_flow);
    ef_convolve_vert(uy2, uu, deriv_flow);
    ef_convolve_vert(vy2, vv, deriv_flow);
...
ef_image_t* ef_compute_dpsis_weight(ef_color_image_t *im, float coef, const convolution_t *deriv) {
    ef_image_t* lum = ef_image_new(im->width, im->height), *lum_x = ef_image_new(im->width, im->height), *lum_y = ef_image_new(
            im->width, im->height);
    int i;
    // ocompute luminance
    ...
    ef_convolve_horiz(lum_x, lum, deriv);
    ef_convolve_vert(lum_y, lum, deriv);
    // compute lum norm
    lump = (v4sf*) lum->data;
    ...
    ef_image_delete(lum_x);
    ef_image_delete(lum_y);
    return lum;
}
