...
ef_image_t** ef_readFlowFile(const char *filename){
    ...
    ef_image_t** flow = (ef_image_t**) malloc(sizeof(ef_image_t*)*2);
    flow[0] = ef_image_new(aXSize, aYSize);
    flow[1] = ef_image_new(aXSize, aYSize);
    int x,y;
    for (y = 0; y < aYSize; y++)
        for (x = 0; x < aXSize ; x++) {
...
static void raw_read_color(FILE *fp, ef_color_image_t *image){
    int j;
...
ef_color_image_t *color_image_pnm_load(FILE *fp){
    ef_color_image_t *image = NULL;
    ppm_hdr_t ppm_hdr;
    ...
    switch(ppm_hdr.magic)    {
        ...
        case 5: /* PGM RAW */
            ...
            break;
        case 6: /* PPM RAW */
            image = ef_color_image_new(ppm_hdr.width, ppm_hdr.height);
...
ef_color_image_t *color_image_jpeg_load(FILE *fp){
    ...
    image = ef_color_image_new(cinfo.image_width, cinfo.image_height);
    if(image == NULL){
        return NULL;
    }
...
ef_color_image_t *color_image_png_load( FILE* fp, const char* file_name ){
    // read the header
    png_byte header[8];
    fread(header, 1, 8, fp);
    ...
    ef_color_image_t* image = ef_color_image_new(temp_width, temp_height);
    if( color_type==0 ) {
...
ef_color_image_t *ef_color_image_load(const char *fname){
    ...
    if((fp = fopen(fname, "rb")) == NULL){
        fprintf(stderr, "Error in ef_color_image_load() - can not open file `%s' !\n", fname);
        exit(1);
    }
    ...
    if(magic_short[0] == 0xd8ff){
      ...
      image = color_image_png_load( fp, fname );
        ...
        fprintf(stderr, "Error in ef_color_image_load(%s) - image format not supported, can only read jpg or ppm\n",fname);
        exit(1);
    }
    fclose(fp);
    return image;
