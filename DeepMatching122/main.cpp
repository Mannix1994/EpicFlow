...
image_t_dm *rescale_image(image_t_dm *im, int width, int height) {
    image_t_dm *res = dm_image_new(width, height);
    dm_image_resize_bilinear_newsize(res, im, width, height);
    dm_image_delete(im);
    return res;
}
...
#define isarg(key)  !strcmp(a,key)
        ...
        if (isarg("-h") || isarg("--help")) dm_usage(language);
    ...
    else if (params->subsample_ref && (!ispowerof2((*im1)->width) || !ispowerof2((*im1)->height))) {
...
int main_deep_matching(int argc, const char **argv) {
    ...
    int current_arg = 3;
    image_t_dm *im1 = NULL, *im2 = NULL;
    {
        color_image_t_dm *cim1 = dm_color_image_load(argv[1]);
        color_image_t_dm *cim2 = dm_color_image_load(argv[2]);
        ...
        im1 = dm_image_gray_from_color(cim1);
        im2 = dm_image_gray_from_color(cim2);
        dm_color_image_delete(cim1);
        dm_color_image_delete(cim2);
    }
    ...
    // save result
    dm_output_correspondences(out_filename, (dm_corres_t *) corres->pixels, corres->ty, fx, fy);
