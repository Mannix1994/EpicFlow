...
void dm_output_correspondences(const char *out_filename, const dm_corres_t *corres, int nb, float fx, float fy) {
    assert(0 < fx && fx <= 2);
    assert(0 < fy && fy <= 2);
...
static void raw_read_color(FILE *fp, color_image_t_dm *image) {
...
color_image_t_dm *color_image_pnm_load(FILE *fp) {
    color_image_t_dm *image = NULL;
    ppm_hdr_t ppm_hdr;
    ...
    switch (ppm_hdr.magic) {
        ...
        case 5: /* PGM RAW */
            ...
            break;
        case 6: /* PPM RAW */
            image = dm_color_image_new(ppm_hdr.width, ppm_hdr.height);
...
color_image_t_dm *color_image_jpeg_load(FILE *fp) {
    ...
    image = dm_color_image_new(cinfo.image_width, cinfo.image_height);
    if (image == NULL) {
        return NULL;
    }
...
color_image_t_dm *color_image_png_load(FILE *fp, const char *file_name) {
    // read the header
    png_byte header[8];
    fread(header, 1, 8, fp);
    ...
    color_image_t_dm *image = dm_color_image_new(temp_width, temp_height);
    if (color_type == 0) {
...
color_image_t_dm *dm_color_image_load(const char *fname) {
    FILE *fp;
    char magic[2];
    unsigned short *magic_short = (unsigned short *) magic;
    color_image_t_dm *image = NULL;
    if ((fp = fopen(fname, "rb")) == NULL) {
        err_printf("Warning: dm_color_image_load() - can not open file `%s' !\n", fname);
        return NULL;
    }
    ...
    if (magic_short[0] == 0xd8ff) {
        ...
        err_printf("Warning: dm_color_image_load(%s) - image format not recognized\n", fname);
    }
    fclose(fp);
    return image;
}
