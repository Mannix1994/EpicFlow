...
void extract_image_desc( image_t_dm* img0, image_t_dm* img1, const dm_params_t* params,
                         float_layers** desc0, float_layers** desc1 )
{
    ...
    dm_image_crop(img0, size[0], size[1]);
    
    // extract gradient-based information
    *desc0 = dm_extract_desc(img0, &params->desc_params, params->n_thread);
    *desc1 = dm_extract_desc(img1, &params->desc_params, params->n_thread);
}
...
void avgpool2( float_layers* hog, const dm_params_t* params )
{
    ...
    while(niter--) {
      float_layers res = empty_layers(float,hog->tx/2,hog->ty/2,hog->tz);
        dm_avgpool2(hog, &res, params->n_thread);
                        ...
                        int_cube* grid, int_cube* children, float_image* norms )
{
    ...
    *grid = empty_cube(int,gtx,gty,2);
    ...
    dm_prepare_big_cells(cell_size, offset, step, child_grid, child_norms, grid, children, norms);
}
                     ...
                     float_image* patches, float_array* norms ) 
{
    ...
    patch_size /= f;
    const int nh = dm_get_patch_desc_dim(hog, patch_size);
    
    assert(!patches->pixels);
    *patches = empty_image(float,nh,npos);
    assert(norms->tx==npos);

    dm_sample_patches(hog, NULL, &new_pos, patch_size, norm, patches, norms, n_thread);
...
void convolve_atomic_patches( float_layers* source, float_layers* target, 
                              const dm_params_t* params, dm_res_scale* first_level )
{
    ...
    dm_fastconv(&patches, target, psize / f, params->ngh_rad / f, extend, norm, params->n_thread, first_level);
...
int_image* maxpool3_and_subsample2( float_layers* hog, int true_shape[2], int_image* offsets, float_layers* res, int nt )
{
  ...
  if ( offsets->pixels == NULL ) {
    ...
    *res = empty_layers(float, true_shape[0], true_shape[1], hog->tz);
      dm_max_filter_3_and_subsample_layers(hog, res, nt);
    return NULL;
    
  } else {
    // with offsets
    float_layers maxpooled_hog = layers_like(float, hog);
      dm_max_filter_3_layers(hog, &maxpooled_hog, nt);
    ...
    *res = empty_layers(float, width, height, hog->tz);
      dm_subsample2_offset(&maxpooled_hog, offsets, res, nt);
                 ...
                 dm_res_scale* res )
{
  ...
  int n = dm_sparse_conv(&_children, _ch_assign, gap, trans_inv, map, offsets,
...
dm_res_scale new_pyramid_level(int f, int psize)
{
  dm_res_scale res = {0};          // initialize everything to 0/NULL
                              ...
                              matching_pyramid_t& res_maps )
{
    ...
    dm_res_scale *child, *last = &res_maps[res_maps.size()-1];
    ...
    if( params->nlpow>0 )
        dm_fastipow(&last->res_map, params->nlpow, params->n_thread);
...
int_image* find_optimal_matchings( matching_pyramid_t& mp, const dm_params_t* params ) 
{
  ...
  if( params->maxima_mode ) { // normal process: maxima detection
    ...
    for(unsigned int i=0; i<mp.size(); i++) sc_maxima.pixels[i]=1;  // useless but well
      ...
      dm_extract_maxima(mp.data(), mp.size(), &sc_maxima, th, params->min_level, params->nlpow,
...
float_image* gather_correspondences( int src_shape[2], int target_shape[2], 
                                   ...
                                   const dm_params_t* params, dm_full_corres_t* corres_out )
{
    ...
    for(i=0; i<n_maxima; i++) {
      ...
      if( params->scoring_mode )  // new mode
          dm_argmax_correspondences(scales.data(), level, num_map, x, y, ((float *) m)[4],
                                    &corres0, step, &corres1, step, i);
      else  // old iccv mode
          dm_argmax_correspondences_v1(scales.data(), level, num_map, x, y, m[0] * ((float *) m)[4],
                                       &corres0, step, &corres1, step, i);
    }
    ...
    int nres;
    float* corres = dm_intersect_corres(&corres0, &corres1, &nres);
...
float_layers* rotate45( float_layers* hog, const dm_params_t* params, dm_full_corres_t* corres_out ) {
...
void dm_set_default_dm_params(dm_params_t *params)
{
  // pixel descriptor params
    dm_set_default_desc_params(&params->desc_params);
...
// main function
float_image* dm_deep_matching(image_t_dm *img0, image_t_dm *img1, const dm_params_t *params,
                              dm_full_corres_t *corres_out)
{
  // verify parameters
...
// set default parameters
void dm_set_default_scalerot_params(dm_scalerot_params_t *params) {
  params->fast = true;
...
float_image* dm_deep_matching_scale_rot(image_t_dm *img0, image_t_dm *img1, dm_params_t *params,
                                        const dm_scalerot_params_t *sr_params) {
  // verify parameters
  ...
  dm_full_corres_t out;
  
  const int NS = 5;
  image_t_dm *scaled_images1[NS] = {NULL};
  ...
  for(int sc0 = sr_params->min_sc0; 
    ...
    image_t_dm* scaled_img0 = ( scale0 >= 1 ) ? img0 :
                              dm_image_resize_bilinear_scale(img0, scale0);
    ...
    for(int sc1 = sr_params->min_sc1; 
      ...
      image_t_dm* scaled_img1 = scaled_images1[sc1 - sr_params->min_sc1];
      if( scaled_img1 == NULL ) {
        scaled_img1 = ( scale1 >= 1 ) ? img1 :
                      dm_image_resize_bilinear_scale(img1, scale1);
        // remember result
      ...
      for(int rotation = sr_params->min_rot;  
        ...
        if( max_dim(scaled_img0) >= max_dim(scaled_img1) ) { // first image is always the largest
          ...
          float_image* corres = dm_deep_matching(scaled_img0, scaled_img1, params, &out);
          ...
          float_image* corres = dm_deep_matching(scaled_img1, scaled_img0, params, &out);
          free_image( corres ); // we don't care
    ...
    // free memory
    if( img0 != scaled_img0 )
        dm_image_delete(scaled_img0);
  }
  
  // final intersection
  int nres;
  float* corres = dm_intersect_corres(&all_corres0, &all_corres1, &nres);
  float_image* res = NEW(float_image);
  *res = (float_image){corres, 6, nres};
  ...
  for(int i=0; i<NS; i++)
    if( scaled_images1[i] != img1 )
        dm_image_delete(scaled_images1[i]);
