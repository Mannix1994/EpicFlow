...
*/
int dm_get_patch_desc_dim(float_layers *hog, int patch_size);
...
*/
void dm_sample_patches(float_layers *hog, float_layers *color, int_image *pos, int size, float norm,
...
*/
void dm_norm_layers(float_layers *res, float norm, int n_thread);
...
*/
void dm_prepare_big_cells(int size, int offset, int step,
...
*/
void dm_fastconv(float_image *patches, float_layers *hog, int patch_size, int ngh_rad,
                 int extend, float norm, int nt, dm_res_scale *res);
...
*/
int dm_sparse_conv(int_image *children, int_array *child_assign, int gap, float trans_inv,
...
/* Compute: arr **= p
*/
void dm_fastipow(float_layers *arr, const float p, int n_thread);

/* Compute: arr = max(0,(arr-p)/(1-p))
*/
void dm_fasthinge(float_layers *arr, const float p, int n_thread);

/* Compute: arr = exp(-arr)
*/
void dm_fastnegexp(float_image *arr);
...
*/
void dm_incorporate_color(int_cube *grid, int_array *assign, float_layers *lab0, float_layers *var0,
