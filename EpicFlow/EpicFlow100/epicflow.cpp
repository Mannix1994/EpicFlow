...
int main_epic_flow(int argc, char **argv){
    ...
    // read arguments
    ef_color_image_t *im1 = ef_color_image_load(argv[1]);
    ef_color_image_t *im2 = ef_color_image_load(argv[2]);
    float_image edges = ef_read_edges(argv[3], im1->width, im1->height);
    float_image matches = ef_read_matches(argv[4]);
