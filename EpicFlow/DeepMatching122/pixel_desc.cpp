...
*/
UBYTE_image* image_to_arraytype( image_t_dm* img ) {
...
// set default params
void dm_set_default_desc_params(desc_params_t_dm *params)
{
...
*/
float_layers* dm_extract_desc(image_t_dm *_img, const desc_params_t_dm *params, int nt)
{
  // verify parameters
  ...
  *hog = {NEWA(float,9*npix),img->tx,img->ty,8};
  ...
  if( params->mid_smoothing )
    dm_smooth_hog_gaussian(hog, params->mid_smoothing, nt);
  //hash_layers(hog)
  ...
  if( params->hog_sigmoid ) {
    ...
    dm_sigmoid_array(&hog_ravel, params->hog_sigmoid, 0, nt);
  }
  //hash_layers(hog)
  ...
  if( params->post_smoothing )
    dm_smooth_hog_gaussian(hog, params->post_smoothing, nt);
  //hash_layers(hog)
