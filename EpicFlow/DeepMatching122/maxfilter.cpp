...
void dm_max_filter_3(float_image *img, float_image *res, int n_thread) {
  _max_filter_3_vert(img,res);
  _max_filter_3_horiz(res,res, res->ty>128? n_thread : 1);
}

void dm_max_filter_3_layers(float_layers *img, float_layers *res, int n_thread) {
  ...
  for(l=0; l<img->tz; l++) {
    ...
    float_image res2 = {res->pixels + l*npix,res->tx,res->ty};
      dm_max_filter_3(&img2, &res2, n_thread);
  }
}
...
*/
void dm_subsample2(float_layers *img, float_layers *res, int n_thread) {
...
*/
void dm_max_filter_3_and_subsample_layers(float_layers *img, float_layers *res, int n_thread) {
  const int n_layers = res->tz;
  assert( img->tz==n_layers );
  const int tx = res->tx;
  const int ty = res->ty;
...
*/
void dm_subsample2_offset(float_layers *img, int_image *offsets, float_layers *res, int n_thread) {
  const int n_layers = res->tz;
  assert( img->tz==n_layers );
...
*/
void dm_maxpool2(float_layers *img, float_layers *res, int n_thread) {
  const int n_layers = res->tz;
  assert( img->tz==n_layers );
  const int tx = res->tx;
  const int ty = res->ty;
...
*/
void dm_avgpool2(float_layers *img, float_layers *res, int n_thread) {
  const int n_layers = res->tz;
  assert( img->tz==n_layers );
  const int tx = res->tx;
  const int ty = res->ty;
...
void dm_get_list_parents(int_cube *children, int_image *res) {
...
static inline int* get_list_parents( int_cube* children, int n_cells_at_prev_scale ) {
  ...
  dm_get_list_parents(children, &res);
  return res.pixels;
}
...
*/
void dm_extract_maxima(dm_res_scale *scales, int n_scales, float_array *sc_factor, float th, int min_scale, float nlpow,
  ...
  for(s=min_scale_max; s<n_scales; s++) {
    ...
    dm_max_filter_3_layers(&r, &sc->max_map, n_thread);
  }
...
*/
void _argmax_correspondences_rec( dm_res_scale* scales, int s, int l, int x, int y,
  ...
  if(s==0) {
    ...
    const int f = sc->f;
    const dm_res_scale* lower = &scales[s-1];
...
void dm_argmax_correspondences(dm_res_scale *scales, int s, int l, int x, int y, float score,
                               float_cube *res0, int step0, float_cube *res1, int step1,
                               int index) {
  assert(res0->tz==6);
...
void _argmax_correspondences_rec_v1( dm_res_scale* scales, int s, int l, int x, int y,
                                  float_cube* res0, int step0, float_cube* res1, int step1, 
...
void dm_argmax_correspondences_v1(dm_res_scale *scales, int s, int l, int x, int y, float top_score,
                                  float_cube *res0, int step0, float_cube *res1, int step1,
                                  int index) {
  assert(res0->tz==6);
  if(res1)  assert(res0->tz==6);
...
*/
float* dm_intersect_corres(const float_cube *map0, const float_cube *map1, int *nres) {
  const int tz = 6;
...
*/
void dm_transfer_corres_score(const float_image *ref, float_cube *map0) {
  const int tz = 6;
...
*/
void dm_merge_corres(float *rot0, float *rot1, int step0, int step1,
