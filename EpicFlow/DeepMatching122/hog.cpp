...
void dm_smooth_gaussian(UBYTE_image *img, float _sigma, UBYTE_image *res, int n_thread) {
...
/* Compute the dx,dy gradient on the image based on a [-1,0,1] mask. 
     ...
     =1 :  sobel smoothing 
*/
void dm_compute_grad_101(UBYTE_image *img, int method, float_layers *grad, int n_thread) {
  ASSERT_SAME_SIZE(img,grad);
  assert(grad->tz==2);
...
*/
void dm_compute_hog(float_layers *grad, int method, float_layers *hog, int n_thread) {
...
*/
void dm_compute_hog_8_direct(UBYTE_image *image, float_layers *hog_out, int n_thread) {
...
*/
void dm_subtract_mean_ori(float_layers *hog, float coef, int n_thread) {
  const int npix = hog->tx*hog->ty;
  int l;
...
*/
void dm_sigmoid_array(float_array *img, float coef, float offset, int n_thread) {
  assert(coef>0);
...
*/
void dm_smooth_hog_gaussian(float_layers *hog, float smoothing, int n_thread) {
  int l;
  const int npix = hog->tx*hog->ty;
  for(l=0; l<hog->tz; l++)
